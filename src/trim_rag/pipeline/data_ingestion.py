...
class DataIngestionPipeline:
    ...
    def text_ingestion(self) -> None:
       try:
            ...
            logger.log_message("info", "Text ingestion pipeline completed successfully.")

       except Exception as e:
            logger.log_message("warning", "Failed to run text ingestion pipeline: " + str(e))
            my_exception = MyException(
    ...
    def image_ingestion(self) -> List[str]:
        try:
            ...
            imageIngestion = ImageIngestion(self.image_data, self.config.image_access_key)
            ...
            logger.log_message("info", "Image ingestion pipeline completed successfully.")
            return image_links


        except Exception as e:
            logger.log_message("warning", "Failed to run image ingestion pipeline: " + str(e))
            my_exception = MyException(
    ...
    def audio_ingestion(self) -> None:
        try:
            ...
            audioIngestion = AudioIngestion(self.audio_data, self.config.audio_access_key)
            audioIngestion.audio_ingestion()

            logger.log_message("info", "Audio ingestion pipeline completed successfully.")

        except Exception as e:
            logger.log_message("warning", "Failed to run audio ingestion pipeline: " + str(e))
            my_exception = MyException(
