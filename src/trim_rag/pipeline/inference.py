...
from src.trim_rag.config  import MultimodalEmbeddingArgumentsConfig, EmbeddingArgumentsConfig, DataTransformationArgumentsConfig
from src.trim_rag.inference import Inference
class InferencePipeline:
    def __init__(self, config: MultimodalEmbeddingArgumentsConfig,
                 config_embedding: EmbeddingArgumentsConfig,
                 config_processing: DataTransformationArgumentsConfig):
        super(InferencePipeline, self).__init__()

        self.infer = Inference(config=config,
    ...
    def run_inference(self, 
                      ...
                      ) -> Union[Optional[List[float]], Optional[List[str]], Optional[List[float]], Optional[List[float]]]:
        try:
            logger.log_message("info", "Inference pipeline started.")
            text_infer_processing, image_infer_processing, audio_infer_processing = self.infer.run_data_processing_pipeline(
                text_path=text,
                image_path=image,
                audio_path=audio
            )

            text_embeddings, image_embeddings, audio_embeddings = self.infer.run_data_embedding_pipeline(
