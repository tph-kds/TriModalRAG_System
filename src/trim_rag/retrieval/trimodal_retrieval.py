...
class TriModalRetrieval:
    def __init__(self, config: TrimodalRetrievalPipelineArgumentsConfig,
        ...
        self.client = QdrantVectorDB(self.config_qdrant,
                                     QDRANT_API_KEY,
                                     QDRANT_DB_URL
                                     )._connect_qdrant()
    ...
    def _init_func(self):
        try:
            ...
            tex_retrieval = TextRetrieval(self.text_retrieval, 
                                          self.config_qdrant, 
                                          self.client)
            img_retrieval = ImageRetrieval(self.image_retrieval, 
                                           self.config_qdrant, 
                                           self.client)
            aud_retrieval = AudioRetrieval(self.audio_retrieval, 
                                           self.config_qdrant,
                                           self.client)
    ...
    def trimodal_retrieval(self,
        ...
        try:
            ...
            if image_embedding_query is not None:
                ...
                vector_image = image_embedding[0].vector # get the vector after retrieval from qdrant
            ...
            if audio_embedding_query is not None:
                ...
                vector_audio = audio_embedding[0].vector # get the vector after retrieval from qdrant
