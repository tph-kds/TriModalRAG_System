...
from src.trim_rag.processing import (
    TextTransform, 
    ImageTransform, 
    AudioTransform
)
...
class TextDataInference(TextTransform):
    ...
    def text_processing(self, input: str) -> Optional[str]:
        try:
            ...
            logger.log_message("info", "Pre-Processing text data in inference phase completed successfully.")
            return text


        except Exception as e:
            logger.log_message("warning", "Failed to run text processing in Inference Phase: " + str(e))
            my_exception = MyException(
                error_message = "Failed to run text processing in Inference Phase: " + str(e),
                error_details = sys,
            )
            print(my_exception)
...
class ImageDataInference(ImageTransform):
    ...
    def image_processing(self, input):
        try: 
            logger.log_message("info", "Pre-Processing image data in inference phase started.")
            ...
            logger.log_message("info", "Pre-Processing image data in inference phase completed successfully.")

            return converted_image

        except Exception as e:
            logger.log_message("warning", "Failed to process image in inference phase: " + str(e))
            my_exception = MyException(
...
class AudioDataInference(AudioTransform):
    ...
    def audio_processing(self, input):
        try:
            logger.log_message("info", "Pre-Processing audio data in inference phase started.")
            ...
            logger.log_message("info", "Pre-Processing audio data in inference phase completed successfully.")

            return audio_finally

        except Exception as e:
            logger.log_message("warning", "Failed to process audio in inference phase: " + str(e))
            my_exception = MyException(
