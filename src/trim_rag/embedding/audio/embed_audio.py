...
class AudioEmbedding:
    ...
    def _get_model(self) -> Optional[Wav2Vec2Model]:
        try:
            ...
            model = Wav2Vec2Model.from_pretrained(pretrained_model_name_or_path=self.pretrained_model_name,
            ...
            logger.log_message("info", "Getting model for embedding audio completed successfully.")
            return model

        except Exception as e:
            logger.log_message("warning", "Failed to get model for embedding audio: " + str(e))
    ...
    def _get_tokenizer(self) -> Optional[Wav2Vec2Tokenizer]:
        try:
            ...
            # Load the tokenizer 
            tokenizer = Wav2Vec2Tokenizer.from_pretrained(pretrained_model_name_or_path=self.pretrained_model_name,
                                                          revision=self.revision,
                                                          trust_remote_code=self.trust_remote_code
                                                          )

            logger.log_message("info", "Getting tokenizer for embedding audio completed successfully.")
            return tokenizer
        
        except Exception as e:
            logger.log_message("warning", "Failed to get tokenizer for embedding audio: " + str(e))
            my_exception = MyException(
    ...
    def _reduce_dimensions(self, embeddings: torch.Tensor) -> Optional[torch.Tensor]:
        try:
            logger.log_message("info", "Reducing dimensions for embedding audio started.")
            pca = PCA(n_components=self.n_components)
            reduced_embeddings = pca.fit_transform(embeddings.squeeze(0))

            logger.log_message("info", "Reducing dimensions for embedding audio completed successfully.")
            return reduced_embeddings
        
        except Exception as e:
            logger.log_message("warning", "Failed to reduce dimensions for embedding audio: " + str(e))
            my_exception = MyException(
                error_message = "Failed to reduce dimensions for embedding audio: " + str(e),
                error_details = sys,
            )
            print(my_exception)
    ...
    def _get_features(self, audio: torch.Tensor) -> Optional[torch.Tensor]:
        try:
            ...
            logger.log_message("warning", "Failed to get features for embedding audio: " + str(e))
            my_exception = MyException(
